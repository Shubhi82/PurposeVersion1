...
def load_v7_dm_differences(path: Path = DM_DIFF_PATH) -> pd.DataFrame:
    """Load the V7 DM-negative-differences file used to build NON-DM applications."""
    workbook = pd.ExcelFile(path, engine="openpyxl")
    sheet_name = "in" if "in" in workbook.sheet_names else workbook.sheet_names[0]
    df = pd.read_excel(workbook, sheet_name=sheet_name, engine="openpyxl")
    df.columns = [str(c).strip().upper() for c in df.columns]

    for col in ["ISO_YEAR", "ISO_WEEK", "APPLICATIONS", "DM_APPLICATIONS", "APPROVED", "ORIGINATIONS"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0.0)

    if "STATE_CD" in df.columns:
        df["STATE_CD"] = df["STATE_CD"].astype(str).str.strip().str.upper()
    if "ORIG_CHANNEL_CD" in df.columns:
        df["ORIG_CHANNEL_CD"] = df["ORIG_CHANNEL_CD"].astype(str).str.strip().str.upper()
...
def build_v7_modeling_frame(channel: str) -> pd.DataFrame:
    ...
    This keeps the original V6/V7 OLS iteration logic, but swaps in the updated
    NON_DM_APPLICATIONS source requested in the newer backend changes.
    """
    ms = pd.read_csv(MARKETING_SPEND_PATH).drop_duplicates()
    ms.columns = [str(c).strip() for c in ms.columns]
    ms["BUSINESS_DATE"] = pd.to_datetime(ms["BUSINESS_DATE"], errors="coerce")
    ms["ISO_YEAR"] = ms["BUSINESS_DATE"].dt.isocalendar().year.astype("Int64")
    ms["ISO_WEEK"] = ms["BUSINESS_DATE"].dt.isocalendar().week.astype("Int64")
    ms["STATE_CD"] = ms["STATE_CD"].astype(str).str.strip().str.upper()
    ms["TOTAL_COST"] = pd.to_numeric(ms["TOTAL_COST"], errors="coerce").fillna(0.0)

    ms_w = (
        ms.groupby(["ISO_YEAR", "ISO_WEEK", "DETAIL_TACTIC", "STATE_CD"], dropna=False)["TOTAL_COST"]
        .sum()
        .reset_index()
    )
    ms_wide = ms_w.pivot_table(
    ...
    period = 52.0
    for k in [1, 2]:
        df[f"sin_{k}"] = np.sin(2 * np.pi * k * df["ISO_WEEK"] / period)
        df[f"cos_{k}"] = np.cos(2 * np.pi * k * df["ISO_WEEK"] / period)
