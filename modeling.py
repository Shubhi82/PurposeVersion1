...
from data_processing import V6_ITERATIONS, build_v7_modeling_frame, fit_v6_iteration
...
def run_v7_iteration_model(channel: str, state: str, iteration_num: int) -> dict | None:
    """
    Convenience wrapper for the V7 NON-DM pipeline.
    Builds the updated V7 modeling frame, filters to one state, and runs the
    requested V6/V7 OLS iteration configuration.
    """
    cfg = next((cfg for cfg in V6_ITERATIONS if cfg["num"] == iteration_num), None)
    if cfg is None:
        raise ValueError(f"Unknown V7 iteration number: {iteration_num}")

    frame = build_v7_modeling_frame(channel)
    entity_df = frame.loc[frame["STATE_CD"] == state].copy()
    entity_df = entity_df.sort_values(["ISO_YEAR", "ISO_WEEK"]).reset_index(drop=True)
    if entity_df.empty:
        return None

    return fit_v6_iteration(
        entity_df,
        channel,
